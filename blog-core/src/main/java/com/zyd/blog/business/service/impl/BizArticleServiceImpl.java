...
public class BizArticleServiceImpl implements BizArticleService {
    ...
    private void subquery(BizArticle entity) {
        ...
        String key = MessageFormat.format("{0}-articleDetail-{1}",CommonConst.WEB_CACHE_SUFFIX,primaryKey.toString());
        boolean hasKey = redisService.hasKey(key);
        boolean needQuery=true;
        if (hasKey) {
            try {
                log.info("{}从缓存中获取数据", key);
                BizArticle cacheeEntity=redisService.get(key);
                if(null!=cacheeEntity){
                    entity.setLookCount(cacheeEntity.getLookCount());
                    entity.setCommentCount(cacheeEntity.getCommentCount());
                    entity.setLoveCount(cacheeEntity.getLoveCount());
                    needQuery=false;
                }
            } catch (Exception e) {
        ...
        if (needQuery) {
          ...
          redisService.set(key, entity, CommonConst.WEB_DEFAULT_EXPIRE, TimeUnit.HOURS);
        }
    }
